...
3.	Para el departamento de logística:
	...
	A.	Es necesario saber la cantidad camiones que harian falta para transportar los materiales
     que se utilizarán para la construcción de un edificio. Para ello, se ingresa la cantidad de toneladas necesarias
    de materiales a transportar. El programa deberá informar la cantidad de camiones, sabiendo que cada uno de 
    ellos puede transportar por viaje 3500kg
...
class App(customtkinter.CTk):
    ...
    def btn_cantidad_camiones_on_click(self):
        ...
        cantidad_camiones_str = self.txt_cantidad_camiones.get() 

        toneladas_int = int(toneladas_str)
        kilometros_int = int(kilometros_str) 
        cantidad_camiones_int = int(cantidad_camiones_str)
        
        result = (toneladas_int / cantidad_camiones_int)
        alert("Calculadora", f"La cantidad de camiones necesarios son: {result}")

    def btn_tiempo_llegada_on_click(self):
        pass 

        #AttributeError: '_tkinter.tkapp' object has no attribute 'txt_cantidad_camiones'
